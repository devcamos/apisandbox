...
export function AwsCertificationProgress({
  ...
  if (status === "loading") {
    return (
      <div className="mt-6 flex items-center gap-2 rounded-2xl border border-slate-800 bg-slate-900/50 p-5 text-sm text-slate-400">
        <LoaderCircle className="h-4 w-4 animate-spin" /> Loading question progress…
      </div>
    )
  }
  ...
  if (status !== "authenticated") {
    return (
      <section className="mt-6 rounded-2xl border border-slate-700 bg-slate-900/60 p-5" aria-label="Question progress">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            <BarChart3 className={`h-5 w-5 ${styles.text}`} />
            <span className="font-semibold text-white">Track your question progress</span>
          </div>
          <Link href="/login" className={`text-sm font-semibold ${styles.text} hover:text-white`}>Sign in to save results</Link>
        </div>
        <p className="mt-2 text-sm text-slate-400">Each mastered question set counts toward your course completion.</p>
      </section>
    )
  }
  ...
  return (
    <section className="mt-6 rounded-2xl border border-slate-700 bg-slate-900/60 p-5 sm:p-6" aria-label="Question progress">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="flex items-center gap-2">
          <BarChart3 className={`h-5 w-5 ${styles.text}`} />
          <div>
            <h2 className="font-semibold text-white">Question progress</h2>
            <p className="mt-1 text-sm text-slate-400">Master each question set to complete the course.</p>
          </div>
        </div>
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2">
        <ProgressMetric
          ...
          value={`${masteredUnits}/${course.units.length} question sets`}
      ...
      <div className="mt-4 flex flex-wrap items-center gap-4 text-xs text-slate-400">
        ...
        <span className="inline-flex items-center gap-1.5"><Circle className="h-3.5 w-3.5 text-slate-500" /> {course.units.length - masteredUnits} remaining</span>
