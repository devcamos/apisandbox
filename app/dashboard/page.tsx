...
export default function DashboardPage() {
  ...
  const [recommendedPhaseId, setRecommendedPhaseId] = useState<number>(1)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      ...
      <LearningPathGuide onProfileChange={(profile) => setRecommendedPhaseId(profile.phaseId)} />
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-8">
          {phases.map((phase) => {
            ...
            const isRecommended = phase.id === recommendedPhaseId
            return (
              <Link key={phase.id} href={href}>
                <div className={`group bg-slate-800/50 backdrop-blur-sm rounded-2xl p-8 border transition-all hover:shadow-2xl hover:scale-105 cursor-pointer relative overflow-hidden ${
                  isRecommended
                    ? "border-cyan-300 shadow-xl shadow-cyan-950/30"
                  ...
                  <div className="relative z-10">
                    <div className="flex items-start justify-between mb-4">
                      ...
                      <div className="flex items-center gap-2">
                        {isRecommended && (
                          <span className="px-2 py-0.5 bg-cyan-400/15 border border-cyan-300/30 rounded text-xs text-cyan-100 font-semibold">
                            Recommended
                          </span>
                        )}
